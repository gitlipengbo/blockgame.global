...
<title><?php echo ($room["roomid"]); ?>号吉祥三公房间</title>
...
<script src="https://res.wx.qq.com/open/js/jweixin-1.2.0.js" type="text/javascript"></script>
  ...
  var title  = '<?php echo ($room["roomid"]); ?>号吉祥三公房间';//分享的标题
  var shareimg = 'http://'+window.location.hostname+'/app/erba.png';//分享的图片
  var desc = '<?php if($room['gz2']): ?>牌型： <?php if(is_array($room['gz2'])): foreach($room['gz2'] as $key=>$one): echo ($one); endforeach; endif; endif; ?> <?php if($room['cm']): ?>筹码:<?php echo ($room['cm']); endif; if($room['sx']): ?>上限:<?php echo ($room['sx']); endif; ?> ';//分享的描述信
...
"></div> 
   ...
   <div id="table" class="table">
    <img src="/app/img/sangong12/table.jpg" class="tableBack" />
...
<?php if((count($room['userlist']) > 0 && $room['js'] == 0) || ($room['js'] > 0 && !$room['userlist'][$user['id']])): ?><div id="valert" class="alert">
   ...
   <div class="mainPart" style="margin-top: -34vh;">
    ...
    <div class="id">
     ...
     <div class="text">
       你的<?php echo ($fangzhu["skinname"]); ?>ID:<?php echo ($user["id"]); ?>
     </div>
    </div>
